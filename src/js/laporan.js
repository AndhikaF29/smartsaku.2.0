...
class ReportManager {
    ...
    async exportToPDF() {
        try {
            ...
            // Header
            doc.setFontSize(18);
            doc.text('Laporan Keuangan SmartSaku', 105, 18, { align: 'center' });

            doc.setFontSize(11);
            doc.text(`Periode: ${document.getElementById('startDate').value} - ${document.getElementById('endDate').value}`, 105, 26, { align: 'center' });
            ...
            // Ringkasan
            doc.setFontSize(13);
            doc.text('Ringkasan', 15, 38);

            const summary = this.calculateSummary();
            doc.setFontSize(11);
            doc.text(`Total Transaksi: ${summary.totalTransactions}`, 15, 46);
            doc.text(`Total Pemasukan: ${this.formatCurrency(summary.totalIncome)}`, 15, 52);
            doc.text(`Total Pengeluaran: ${this.formatCurrency(summary.totalExpense)}`, 15, 58);
            doc.text(`Net Worth: ${this.formatCurrency(summary.netWorth)}`, 15, 64);
            ...
            doc.text('Tanggal', 17, y);
            doc.text('Kategori', 40, y);
            doc.text('Jenis', 85, y);
            doc.text('Nominal', 110, y, { align: 'left' });
            doc.text('Deskripsi', 140, y);

            y += 4;

            // Semua data transaksi, urut dari terbaru ke terlama
            // Sudah urut dari TransactionService (tanggal descending)
            this.filteredData.forEach(transaction => {
                if (y > 275) {
                    doc.addPage();
                    y = 20;
                ...
                doc.setFontSize(10);
                doc.text(this.formatDate(transaction.tanggal), 17, y);
                doc.text(transaction.kategori, 40, y);
                doc.text(transaction.jenis.charAt(0).toUpperCase() + transaction.jenis.slice(1), 85, y);
                doc.text(this.formatCurrency(transaction.nominal), 110, y, { align: 'left' });
                doc.text((transaction.deskripsi || '-').substring(0, 35), 140, y);
                // Garis bawah
            ...
            // Footer
            doc.setFontSize(9);
            doc.text('Generated by SmartSaku', 15, 287);
